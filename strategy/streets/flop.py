...
def recommend_flop(features: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    state = build_postflop_state(features, ctx, "flop")
    hydrate_ctx_from_postflop_state(ctx, state)

    add_villain_range_insights(ctx, features, min_board_cards=3, label="Flop")
...
def _handle_open_action(features: Dict[str, Any], ctx: Dict[str, Any], adv_data: Dict[str, Any], state: Dict[str, Any]):
    ...
    reasons = []
    matrix = {"check": 1.0}
    add_line_state_reason(reasons, state.get("line_state", ""))
    ...
    villain_pos = state["villain_pos"] or features.get("villain_pos", "BB")
    ...
    if has_initiative or checked_to_hero:
        if villain_synergy >= 30 and range_adv < 1.2:
            ...
            reasons.append("缺乏足夠優勢與成手，保留部分過牌頻率。")

        if checked_to_hero and not has_initiative:
            matrix = rebalance_bet_check_matrix(matrix, 0.2)
            sizing_ratio = min(sizing_ratio, 0.5)
            reasons.append("對手先放棄主動權，但 Hero 並非原始進攻方，改用較保守的 stab/probe 頻率。")
    else:
        if nut_adv >= 1.3:
            ...
            reasons.append("身為防守方且尚未被 check 到，預設採取過牌。")
